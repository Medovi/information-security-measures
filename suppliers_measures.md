...
| № | Интерфейс | Назначение | Тип интерфейса | Категории | Справка |
|---|-----------|------------|----------------|-----------|---------|
| 1. | /etc/udev/rules.d/*.rules | Блокировка или разрешение конкретных устройств по VID (Vendor-ID)/PID. | Конфигурационные файлы. | USB-устройства. | udev(7) |
| 2. | /sys/bus/usb/devices/*/autorized | Динамическое отключение конкретного USB-порта. | Системный файл устройств.<br>Конфигурационные файлы. | USB-устройства. | usb-devices(1) |
| 3. | /usr/bin/lsusb<br>/usr/bin/usbguard | Просмотр подключённых USB-устройств. | Команды.<br>Демон. | USB-устройства. | lsusb(8)<br>[Alt Wiki: USBGuard](https://www.altlinux.org/USBGuard) |
| 4. | /etc/usbguard/rules.conf | Политики доступа к USB-устройствам с поддержкой криптоподписи. | Конфигурационный файл. | USB-устройства. | [Alt Wiki: USBGuard](https://www.altlinux.org/USBGuard) |
| 5. | /etc/polkit-1/rules.d/* | Контроль прав доступа к операциям с устройствами (например, монтирование). | Polkit правила на JS. | USB-устройства. | polkit(8) |
| 6. | /etc/fstab<br>Значение полей: noauto,user | Запрет автоматического монтирования съёмных носителей. | Конфигурационный файл. | USB-устройства. | fstab(5) |
| 7. | /etc/udisks2/*.conf | Тонкая настройка политик монтирования через udisks | Конфигурационные файлы. | USB-устройства. | udisks2.conf(5) |
| 8. | /etc/samba/smb.conf<br>Параметр: veto files | Блокировка исполняемых файлов на сетевых ресурсах. | Конфигурационный файл. | Сетевые ресурсы.<br><br>USB-устройства. | smb.conf(5) |
| 9. | /etc/pam.d/system-auth<br>Библиотека: pam_usb.so | Аутентификация по USB-токенам. | PAM-модуль. | USB-устройства. | pam_systemd(8) |
| 10. | /etc/audit/rules.d/usb.rules | Логирование операций с USB-устройствами. | Правила auditd. | USB-устройства. | auditd(8) |
| 11. | /sbin/cryptsetup luksFormat | Шифрование съёмных носителей. | Команда. | USB-устройства. | cryptsetup(8) |
| 12. | /etc/security/namespace.conf | Изоляция пользовательских сессий для съёмных носителей. | Конфигурационный файл. | USB-устройства. | namespace.conf(5) |
| 13. | /usr/sbin/semanage fcontext | Настройка SELinux-контекста для съёмных устройств. | Команда. | USB-устройства. | semanage(8)<br>semanage.conf(5) |
| 14. | /usr/bin/ipa hbacrule-add | Централизованное управление доступом в домене. | Команда. | Доменные пользователи<br><br>USB-устройства. | ipa(1) |
...
| № | Интерфейс | Назначение | Тип интерфейса | Категории | Справка |
|---|-----------|------------|----------------|-----------|---------|
| 1. | /etc/grub.d/40_custom | Настройка GRUB: добавление пароля и пользовательских записей загрузки. | Конфигурационный файл. | Загрузка системы. | [Alt Wiki: Grub](https://www.altlinux.org/Grub) |
| 2. | /usr/bin/grub-mkpasswd-pbkdf2 | Создание зашифрованного пароля GRUB (PBKDF2 hash). | Команда. | Загрузка системы. | [Alt Wiki: Grub](https://www.altlinux.org/Grub) |
| 3. | /boot/grub/grub.cfg | Генерация основного загрузочного меню GRUB. | Конфигурационный файл. | Загрузка системы. | [Alt Wiki: Grub](https://www.altlinux.org/Grub) |
| 4. | /usr/sbin/grub-mkconfig | Команда генерации grub.cfg из шаблонов. | Команда. | Загрузка системы. | [Alt Wiki: Grub](https://www.altlinux.org/Grub) |
| 5. | BIOS/UEFI Setup | Отключение загрузки с USB/сетевых устройств, установка пароля. | Настройка микропрограммы. | Загрузка системы. | [Alt Wiki: https://www.altlinux.org/UEFI](https://www.altlinux.org/UEFI) |
| 6. | Secure Boot | Запрет запуска неподписанных загрузчиков через UEFI. | UEFI-функция. | Загрузка системы. | [Alt Wiki: https://www.altlinux.org/UEFI](https://www.altlinux.org/UEFI) |
| 7. | /sbin/cryptsetup | Настройка шифрования LUKS на дисках. | Команда. |  | cryptsetup(8) |
| 8. | /usr/sbin/aide | Инструмент обнаружения вторжений с механизмом проверки целостности системных файлов. | Команда. | Целостность файлов. | aide(1) |
| 9. | /etc/aide.conf | Конфигурация для AIDE: какие файлы проверять. | Конфигурационный файл. | Целостность файлов. | aide.conf(5) |
| 10. | initrd | Ранняя загрузка и подготовка системы: монтирование LUKS, проверка rootfs. | Механизм загрузки ОС. | Загрузка системы. | initrd(4) |
| 11. | IMA/EVM | Контроль целостности файлов ядра и исполняемых файлов в ранней загрузке. | Ядро и политики. | Целостность файлов. | [https://www.altlinux.org/IMA_EVM](Alt Wiki: IMA/EVM) |
